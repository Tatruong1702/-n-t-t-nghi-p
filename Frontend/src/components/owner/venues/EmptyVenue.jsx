...
const EmptyVenue = React.memo(function EmptyVenue({ onCreate }) {
  return (
    <div className="rounded-[28px] border border-dashed border-slate-300/70 bg-gradient-to-br from-slate-50 via-white to-slate-100 p-10 text-center shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)]">
      ...
      <p className="mx-auto mt-3 max-w-md text-sm text-slate-600">Bắt đầu bằng việc thêm sân đầu tiên và quản lý toàn bộ vận hành của bạn ở một nơi.</p>
      <button
        onClick={onCreate}
        className="mt-8 inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-slate-800"
      >
